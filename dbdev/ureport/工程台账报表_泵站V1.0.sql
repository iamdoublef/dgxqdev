select * from user_tab_comments where comments like '%泵站%';
...
select 
--泵站名称,工程所在地	,工程所在堤围,设计流量（m3/s）,装机容量（台/千瓦）,设计标准,工程管理单位,管理人员（人）	
tss.SK_MC,tss.GCDZ,tss.GCTW,tss.JYMJ,tss.ZKR ,tss.BX, tss.GCGLDW_BH,tss.GLRYSL,
--问题清单
...
tss.FXXZFZR_QM,tss.FXXZFZR_SJ,tss.FXJSZRR_QM
from t_account_info tai
left join t_standbook_bzhan  tss on tai.tz_bh = tss.id and tai.tz_table_name = 't_standbook_bzhan'
where  tai.id = '7b7a3ce448b6488caaab54861fb96d1f'

select * from t_standbook_bzhan;
