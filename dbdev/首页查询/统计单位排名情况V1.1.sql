------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------统计工程报送单位的排名情况-----------------------------------------------------------
--通过T_SEND_INFO报送痕迹表T_ACCOUNT_INFO 台账预制表关联起来，如果当前机构接收成功，表示可以展
WITH DW_WT_STR AS 
(  SELECT
 ACC.ID, ACC.TB_DWBH, ACC.TZ_BH --问题用逗号分开从这里可以获得问题数
...
DW_WT_ZT AS (
  ...
  FROM  (SELECT TB_DWBH,WT_BH FROM DW_WT_LB GROUP BY TB_DWBH,WT_BH) DWL --去重复数
...
--测试数据
------------------------------------------------------------------------------------------------------------------------------------
SELECT '25' officeId,'安监' officeName, 20 sumCount,20 finish,10 doing,1 officeOrder
FROM dual 
union all 
SELECT '20' officeId,'城管' officeName, 50 sumCount,40 finish,10 doing,2 officeOrder
FROM dual 
union all 
SELECT '22' officeId,'公安' officeName, 50 sumCount,40 finish,10 doing,2 officeOrder
FROM dual 
